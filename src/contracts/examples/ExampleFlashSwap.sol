...
// 
    constructor(address _factory, address _factoryV1, address router) public {
        ...
        //Getting the router to be able to access the WETH on v2
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external  {
        ...
        //THIS IS A BIT WEIRD, WE HAVE AN ALREADY EXISTING TOKEN PAIR IN UNISWAPV2LIBRARY YET DEVELOP THE TOKEN AT IUNISWAPV2PAIR
         ...
         // to create pair and pair them hence the need for v1 router and pairer
   ...
   // We make a pair for
        ...
        // PASS THE SWAP ON THE EXCHANGE TO BE SENT OVER TO V1
        ...
        if (amountToken > 0) {
            ...
            (bool success,) = sender.call( amountReceived - amountRequired)(new bytes(0)); // keep the rest! (ETH)
            assert(success);
        } else {
            (uint minTokens) = abi.decode(data, (uint)); // slippage parameter for V1, passed in by caller
            WETH.withdraw(amountETH);
            uint amountReceived = exchangeV1.ethToTokenSwapInput(amountETH)(minTokens, uint(-1));
